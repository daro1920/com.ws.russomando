...
namespace WSClient.models
{
    class Llamados : Servicio
    {
        ...
        public override void updateCanceledServicios(String id)
        {
            ...
            {
                command.CommandText = @"update g:\principal\llamadosc set nroserv = 0  where LLAID = " + id + " ";
        ...
        private void updateLlamadosR(List<DataRow> rowList,JObject servicio)
        {
            ...
            using (OleDbCommand command2 = connectionHandler.CreateCommand())
            {
                foreach (DataRow row in rowList)
                {
                    ...
                    row["LLAOBS"] + " / " + obs.ToUpper() + "', " + clavefinal.Substring(0,1) + " , " + row["LLADEM"] + ",iif('TRUE'='" + row["DIAPRE1"] + "',.t.,.f.),iif('TRUE'='" + row["DIAPRE2"] + "',.t.,.f.),iif('TRUE'='" + row["DIAPRE3"] + "',.t.,.f.),iif('TRUE'='" + row["DIAPRE4"] + "',.t.,.f.),iif('TRUE'='" + row["DIAPRE5"] + "',.t.,.f.) , '" +
